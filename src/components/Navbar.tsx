    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 border-b border-transparent ${scrolled
                ? 'bg-background/90 backdrop-blur-xl border-primary/10 shadow-lg py-2'
                : 'bg-transparent py-4'
                }`}
        >
            <div className="container mx-auto px-6 flex justify-between items-center">
                ...
                <Link to="/" className="block">
                    <img src="/logo.png" alt="Sportive Hub" className="h-12 w-auto" />
                </Link>
                ...
                <div className="hidden md:flex items-center space-x-8">
                    <Link
                        to="/#home"
                        className="text-text hover:text-primary font-medium text-sm uppercase tracking-widest relative group"
                    >
                        Home
                    ...
                    >
                        <Link
                            to="/services"
                            className="text-text hover:text-primary font-medium text-sm uppercase tracking-widest flex items-center gap-1 py-4"
                        >
                        ...
                        <AnimatePresence>
                            {isServicesOpen && (
                                <motion.div
                                    ...
                                    className="absolute top-full left-0 mt-0 w-56 bg-background/95 backdrop-blur-xl border border-primary/10 shadow-2xl py-2 rounded-md overflow-hidden"
                                >
                                    {servicesList.map((service) => (
                                        <Link
                                            key={service.name}
                                            to={service.href}
                                            className="block px-6 py-3 text-sm text-text hover:text-primary hover:bg-primary/5 transition-colors uppercase tracking-wider"
                ...
                {isOpen && (
                    ...
                    >
                        <div className="flex flex-col p-6 space-y-6">
                            <Link
                                to="/#home"
                                className="text-text hover:text-primary font-heading font-bold text-xl uppercase tracking-widest"
                            ...
                            <div className="flex flex-col space-y-4">
                                <Link
                                    to="/services"
                                    className="text-text hover:text-primary font-heading font-bold text-xl uppercase tracking-widest flex items-center gap-2"
                                    onClick={() => setIsOpen(false)}
                                >
                                    Services
                                </Link>
                                <div className="pl-6 flex flex-col space-y-3 border-l-2 border-primary/10 ml-2">
                                    {servicesList.map((service) => (
                                        <Link
                                            key={service.name}
                                            to={service.href}
                                            className="text-text hover:text-primary font-medium text-sm uppercase tracking-wider"
                                            onClick={() => setIsOpen(false)}
                                        >
                                            {service.name}
                                        </Link>
